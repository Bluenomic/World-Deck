...
export const CardReaderSidebar: React.FC<CardReaderSidebarProps> = ({
  ...
  const [isExpanded, setIsExpanded] = React.useState<boolean>(initialFullPage);
  ...
  const containerClasses = isExpanded
    ? 'fixed inset-0 z-[150] bg-black/70 backdrop-blur-sm flex items-center justify-center p-3 md:p-6 animate-in zoom-in-95 duration-200'
    : 'fixed inset-y-0 right-0 z-[120] w-full max-w-lg app-bg-main border-l app-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-300';

  const cardClasses = isExpanded
    ? 'w-full max-w-4xl h-full max-h-[90vh] app-bg-main border app-border rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in duration-200'
    : 'w-full h-full flex flex-col overflow-hidden';

  return (
    <div className={containerClasses} onClick={isExpanded ? onClose : undefined}>
        ...
        <div className="px-5 py-4 border-b app-border app-bg-secondary flex items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              title={isExpanded ? 'Kecilkan ke Sidebar' : 'Buka Kartu Sepenuhnya (Layar Penuh)'}
            >
      ...
      {/* Main Content Body (Scrollable Wiki Article) */}
